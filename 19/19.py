...
### north pole onsen town

inputa = """r, wr, b, g, bwu, rb, gb, br"""
...
tokens = inputa.replace(' ','').split(",")
tokens = inputc.replace(' ','').split(",")


towels = inputb.splitlines()
towels = inputd.splitlines()

def can_create_towel(towel, tokens):
    # Sort tokens by length (longest first for greedy matching)
    tokens = sorted(tokens, key=len, reverse=True)

    # Boolean array to track covered indexes
    covered = [False] * len(towel)

    # Sliding window matching
    for token in tokens:
        #token = token.strip #dont need anymore added to 420
        start = 0
        while start <= len(towel) - len(token):
            # Check if token matches and doesn't overlap uncovered characters
            if towel[start:start + len(token)] == token:
                # Mark matched range as covered
                for i in range(start, start + len(token)):
                    covered[i] = True
            start += 1  # Move window to the right
...
results = []
for towel in towels:
    results.append(can_create_towel(towel, tokens))

# Print results
possible = 0
for i, (towel, result) in enumerate(zip(towels, results)):
    print(f"Towel {i + 1} ({towel}): {'Matchable' if result else 'Not Matchable'}")
    if result:
        possible +=1
